  ...
  try {
    ...
    const story = new Story({
        ...
        // status: req.body.status || 'Open',
            // Check if the status property is defined before setting it
...
//     // Create new Task objects in the database
//     const tasks = await Task.insertMany(req.body.tasks);
//     story.tasks = tasks.map(task => task._id);

//     // Save the Story object to the database
//     await story.save();

//     // Create a worker for each Task in the story
//     const workers = tasks.map(task => {
//       return new Worker('./storyWorker.js', {
//         workerData: { task }
//       });
//     });

//     // Listen for completion message from each worker
//     let numCompleted = 0;
